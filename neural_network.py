    ...
    def update_parameters(self, t):    
        if self.optimizer == 'sgd':                    
            ...
            for l in range(1, self.num_layers+1):
                ...
                #self.learning_rate*(_mW/(np.sqrt(_vW)+self.eps_for_v)), self.epsilon
                self.parameters["W%s" % l] -= self.alpha * m_corrected_W/(np.sqrt(v_corrected_W) + self.epsilon)    #for weights
                self.parameters["b%s" % l] -= self.alpha * m_corrected_b/(np.sqrt(v_corrected_b) + self.epsilon)    #for biases
